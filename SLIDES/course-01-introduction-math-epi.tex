...
% 
% \begin{frame}{Where I am coming from (physically)}
% \bbullet Winnipeg (850K metro area) is the capital of Manitoba (1.5 million)
% \vfill
% \bbullet Winnipeg (Western Cree for ``muddy water'')
% \vfill
% \bbullet 12.5\% of the population is Indigenous (First Nations, Métis, Inuit), 11.6\% Southeast Asian, 7.8\% South Asian, 5\% African, 3.5\% East Asian. Tagalog is the second most common mother tongue. 25.4\% of inhabitants were born outside Canada
% \vfill
% \bbullet The University of Manitoba was founded in 1877 (the Collège universitaire de Saint-Boniface - now Université de Saint Boniface - was founded in 1818). UM has $>$30K students, is part of U15. Closely associated with National Microbiology Laboratory (Canada's only BSL 4)
% \end{frame}
...
% \begin{frame}{Where I am coming from (scientifically)}
% \bbullet MSc (DEA) at R\'eseau Sentinelles (Paris)
% \vfill
% \bbullet PhD at INRIA Sophia Antipolis \& Observatoire du Plancton Marin of Villefranche sur Mer working on large systems of DE and ODE
% \vfill
% \bbullet Postdoc with Pauline van den Driessche in 2001-2002 working on DDE and large systems of ODE
% \vfill
% \bbullet From 2004, collaboration with Kamran Khan, physician specialist in infectious diseases (TB). Collaborated on Bio.Diaspora (project that uses data science to predict and prevent the spread of infectious diseases). Now \href{https://www.blue-dot.global/}{BlueDot}, a company 
% \vfill
% \bbullet Have worked on Emergency Response on SARS-1 (2003), p-H1N1 (2009) and COVID-19 (2020) (and less on Zika, Ebola, MERS, etc.) for the Public Health Agency of Canada
% \end{frame}
...
\bbullet There are three slide sets. Each slide set contains \emph{way} too many slides for a single lecture, I will cover only a subset. Use the rest if you want to learn more. All the work cited in a slide set is collected in a Bibliography at the end of the slide set
\vfill
\bbullet Some slides show \texttt{R} or \texttt{julia} code used to produce the figures shown; they can safely be ignored if you have no interest in numerics (you should, but who am I to say!)
\vfill
\bbullet As much as possible, I am homogeneising notation, denoting similar parameters with the same greek/roman letter
\vfill
...
\end{quote}
\vfill
(Paper worth reading if just for the Introduction)
\end{frame}
