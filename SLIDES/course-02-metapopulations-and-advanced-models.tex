...
% The section page
\newSectionSlide{FIGS/Gemini_Generated_Image_ypohksypohksypoh.jpeg}
...
% The section page
\newSectionSlide{FIGS/Gemini_Generated_Image_bla657bla657bla6.jpeg}

% JA</ins>, N. Bajeux, S. Portet & J. Watmough. [Quarantine and the risk of COVID-19 importation](http://dx.doi.org/10.1017/S0950268820002988). *Epidemiology & Infection* (2020)

\maxFrameImage{FIGS/figure_variant_importation_base_model_with_stimulations_noQ.png}
\maxFrameImage{FIGS/successful_attack_rate.png}
%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%
...
% The section page
\newSectionSlide{FIGS/Gemini_Generated_Image_bla659bla659bla6.jpeg}
...
% The section page
\newSectionSlide{FIGS/Gemini_Generated_Image_bla65bbla65bbla6.jpeg}
...
\begin{center}
	\begin{tikzpicture}[scale=1, every node/.style={transform shape},
		...
		\node [box] at (6,2) (R) {$R$};
		%% Flows
		\path [line, thick] (-1.5,2) to node [midway, above] (TextNode) {$\mathcal{B}(N)$} (S);
...
\hldef{tspan} \hlkwb{=} \hlkwd{seq}\hldef{(}\hlkwc{from} \hldef{=} \hlnum{0}\hldef{,} \hlkwc{to} \hldef{=} \hlnum{5} \hlopt{*} \hlnum{365.25}\hldef{,} \hlkwc{by} \hldef{=} \hlnum{0.1}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{frame}
...
\hlkwa{for} \hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hldef{p}\hlopt{$}\hldef{P) \{}
  \hldef{p}\hlopt{$}\hldef{beta[i]} \hlkwb{=}
    ...
    \hldef{(p}\hlopt{$}\hldef{espilon[i]}\hlopt{*}\hldef{p}\hlopt{$}\hldef{d[i])}
\hldef{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{SLIAR_metapop_rhs} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{t}\hldef{,} \hlkwc{x}\hldef{,} \hlkwc{p}\hldef{) \{}
...
\end{alltt}
\begin{verbatim}
## [1] "../CODE/course-02-metapopulations-and-advanced-models.R"
\end{verbatim}
\end{kframe}
\end{knitrout}
