...
\bbullet There are three slide sets. Each slide set contains \emph{way} too many slides for a single lecture, I will cover only a subset
\vfill
\bbullet However, use the rest if you want to learn more. In particular, some slides show explicitly \texttt{R} or \texttt{julia} code used to produce the figures shown; they can safely be ignored if you have no interest in numerics (you should, but who am I to say!)
\vfill
