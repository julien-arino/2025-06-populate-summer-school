...
<<parallel-CTMC-run-parallel, message=FALSE, warning=FALSE>>=
nb_cores <- detectCores()
if (nb_cores > 124) {
  nb_cores = 124
} else {
  nb_cores = nb_cores-1 # Keep one core empty to avoid freeze up
}
cl <- makeCluster(nb_cores) # Make a cluster with nb_cores workers
clusterEvalQ(cl,{
  # Make sure each worker has the SSA solver available, since the 
  # function run_one_sim uses it
  library(GillespieSSA2) 
})
gamma = 1/3
R0 = 1.5
...
params <- list(gamma = gamma, beta = beta,
               ...
               t_f = 100, nb_sims = 50)
# Make sure each worker has access to what it needs
clusterExport(cl,
              c("params",
                "run_one_sim"),
              envir = .GlobalEnv)
...
stopCluster(cl) # Liberate the workers
# Store in global environment to ensure availability outside of this
# Beamer frame
assign("SIMS", SIMS, envir = .GlobalEnv)
@

<<parallel-CTMC-plot, results='hide', include=FALSE, echo=FALSE>>=
