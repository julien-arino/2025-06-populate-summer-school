...
% The section page
\newSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_ypohksypohksypoh.jpeg}
...
\maxFrameImage{FIGS/Screenshot from 2024-06-25 14-43-05.png}

\maxFrameImage{FIGS/Screenshot from 2024-06-26 08-25-07.png}
...
\maxFrameImage{FIGS/Screenshot from 2024-06-27 00-33-22.png}
\maxFrameImage{FIGS/nutrition-north-canada-eligibility.png}
\maxFrameImage{FIGS/Screenshot from 2024-06-27 00-11-30.png}
...
\item The distribution of waning times is exponential, which leads to
 an ODE system
\vfill
\item The waning time is a constant, i.e., $P(t)$ is Diract distributed, which leads to a discrete DDE model
\end{itemize}
\end{frame}
...
% The section page
\newSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_bla65bbla65bbla6.jpeg}
...
\section{Conclusions}
% The section page
\newSectionSlide{FIGS/Gemini_Generated_Image_hz99k4hz99k4hz99.jpeg}
...
\frametitle{A Long Function Definition}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: params is not a numeric or integer vector}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'sol' not found}}
\end{knitrout}
...
\hldef{\}}
\hldef{cl} \hlkwb{<-} \hlkwd{makeCluster}\hldef{(nb_cores)}
...
\begin{verbatim}
## [[1]]
## [1] "GillespieSSA2" "stats"         "graphics"      "grDevices"    
...
\hldef{SIMS} \hlkwb{=} \hlkwd{parLapply}\hldef{(}\hlkwc{cl} \hldef{= cl,}
                 \hlkwc{X} \hldef{=} \hlnum{1}\hlopt{:}\hldef{params}\hlopt{$}\hldef{number_sims,}
                 \hlkwc{fun} \hldef{=}  \hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlkwd{run_one_sim}\hldef{(params))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in 1:params\$number\_sims: argument of length 0}}\begin{alltt}
...
\begin{frame}
    ...
    \bigskip
    \begin{center}
        \includegraphics[width=0.7\textwidth]{https://placehold.co/800x300/e2e8f0/475569?text=Generation+0 \to Generation+1 \to Generation+2}
        \caption{A simple population tree.}
    \end{center}
...
% --- SLIDE 4: The Most Important Number: Mean Offspring ---
\begin{frame}
    \frametitle{The Most Important Number: Mean Offspring}
    
    The entire fate of the population hinges on a single parameter: the mean of the offspring distribution.
    ...
    \begin{block}{Expected Population Size}
        Using the law of total expectation, we can find the expected size of the next generation:
