...
\begin{frame}[fragile]{Last one did not go well}
    \begin{itemize}
        ...
        \item Interrupted at 294.8520021 because of the test
        \newline (Slide with $b<d$: sim stopped because the population went extinct, I did not stop it!)
        \item At stop time
        \begin{itemize}
            \item $N =$ \ensuremath{4.3315\times 10^{4}}
            \item $|N| =$ 86764 (and $|t|$ as well, of course!)
...
\end{alltt}
\begin{verbatim}
## [1] 7.242383e-04 1.498996e-04 6.954466e-04 5.601957e-04 4.301762e-04
## [6] 4.408662e-09
\end{verbatim}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `map()`:\\\#\# i In index: 1.\\\#\# Caused by error in `sourceCpp()`:\\\#\# ! Error 1 occurred building shared library.}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'sol' not found}}\end{kframe}
\end{knitrout}
...
% <<export-run-one-sim, results='hide'>>=
% # Store in global environment to ensure availability outside of this
% # Beamer frame
% assign("run_one_sim", run_one_sim, envir = .GlobalEnv)
% @
...
\hlcom{# --- FIX: Point to the correct environment for the export ---}
\hlkwd{clusterExport}\hldef{(cl,}
              \hlkwd{c}\hldef{(}\hlsng{"params"}\hldef{,} \hlsng{"run_one_sim"}\hldef{),}
              \hlkwc{envir} \hldef{=} \hlkwd{environment}\hldef{())}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in clusterExport(cl, c("{}params"{}, "{}run\_one\_sim"{}), envir = environment()): could not find function "{}clusterExport"{}}}\begin{alltt}
\hlcom{# # Make sure each worker has access to what it needs}
\hlcom{# clusterExport(cl,}
\hlcom{#               c("params",}
\hlcom{#                 "run_one_sim"),}
\hlcom{#               envir = .GlobalEnv)}
...
\hlcom{# Store in global environment to ensure availability outside of this}
\hlcom{# Beamer frame}
\hlkwd{assign}\hldef{(}\hlsng{"SIMS"}\hldef{, SIMS,} \hlkwc{envir} \hldef{= .GlobalEnv)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'SIMS' not found}}\end{kframe}
\end{knitrout}
