...
\end{itemize}
\begin{center}
\includegraphics[width=0.45\textwidth]{FIGS/distrib_a_b}
\end{center}
\end{frame}
...
\]
\begin{center}
\includegraphics[width=0.45\textwidth]{FIGS/cdf_auc}
\end{center}
\end{frame}
...
\begin{frame}
    ...
    \begin{block}{Continuous-Time Markov Chains (CTMCs)}
        ...
        \begin{itemize}
            \item The time spent in a state is a \textbf{continuous random variable}.
            \item This "holding time" in any state $i$ follows an \textbf{Exponential distribution}, parameterised by an \textit{exit rate} $q_i$.
            \item This is a direct consequence of the Markov Property being applied to continuous time. The exponential is the only continuous distribution with the "memoryless" property.
        \end{itemize}
    \end{block}
\end{frame}

% --- SLIDE 3: RATES AND THE GENERATOR MATRIX ---
\begin{frame}
    \frametitle{The Engine: Transition Rates and the Q-Matrix}
    The dynamics of a CTMC are defined by \textbf{transition rates}, not probabilities.
    ...
    \begin{definition}[Transition Rates]
        ...
        \begin{itemize}
            ...
            \item The total \textbf{exit rate} from state $i$ is $q_i = \sum_{j \neq i} q_{ij}$.
            \item The time spent in state $i$ is an exponential random variable $T_i \sim \text{Exp}(q_i)$.
        \end{itemize}
    \end{definition}
    
    \begin{block}{The Generator Matrix (Q-Matrix)}
        We collect all transition rates into a single matrix $Q$:
        \begin{itemize}
            \item \textbf{Off-diagonals:} $Q_{ij} = q_{ij}$ for $i \neq j$. (The rate of going from $i$ to $j$)
            \item \textbf{Diagonals:} $Q_{ii} = -q_i = -\sum_{j \neq i} q_{ij}$. (The negative of the total exit rate from $i$)
        \end{itemize}
        A key property is that all rows of $Q$ sum to zero: $\sum_{j} Q_{ij} = 0$.
    \end{block}
    
    \begin{example}[A Simple On/Off System]
        A machine breaks down at rate $\alpha$ and is repaired at rate $\beta$.
        \\ States: \{1 (On), 2 (Off)\}. Rates: $q_{12} = \alpha$, $q_{21} = \beta$.
        \vspace{0.5em}
        \\ Exit rates: $q_1 = \alpha$, $q_2 = \beta$.
        \\
        $Q = \Qmatrix{-\alpha & \alpha \\ \beta & -\beta}$
    \end{example}
\end{frame}


% --- SLIDE 4: EVOLUTION OVER TIME ---
\begin{frame}
    \frametitle{Kolmogorov's Differential Equations}
    Let $P(t)$ be the matrix where $P_{ij}(t) = \mathbb{P}(X(t)=j | X(0)=i)$. How does $P(t)$ evolve over time?
    \vspace{1em}
    
    \begin{alertblock}{The Kolmogorov Forward Equations}
        This equation describes the rate of change of probability of ending up in a target state $j$.
        $$ \frac{d}{dt}P(t) = P(t)Q $$
        In element form:
        $$ P'_{ij}(t) = \sum_{k} P_{ik}(t) Q_{kj} $$
        \textbf{Intuition:} The change in probability of being in state $j$ is (Prob. Flow \textit{into} $j$) - (Prob. Flow \textit{out of} $j$).
    \end{alertblock}
    
    \begin{block}{Formal Solution}
        Similar to the simple ODE $y' = ay \implies y(t) = e^{at}y(0)$, the solution is the \textbf{matrix exponential}:
        $$ P(t) = e^{tQ} = \sum_{k=0}^{\infty} \frac{(tQ)^k}{k!} $$
        This shows how the generator matrix $Q$ "generates" the process's evolution.
    \end{block}
\end{frame}


% --- SLIDE 5: LONG-TERM BEHAVIOR ---
\begin{frame}
    ...
    \begin{example}[On/Off System Revisited]
       Solve $\boldsymbol{\pi} Q = \mathbf{0}$ for $\boldsymbol{\pi} = (\pi_1, \pi_2)$:
       $(\pi_1, \pi_2) \Qmatrix{-\alpha & \alpha \\ \beta & -\beta} = (0, 0) \implies -\alpha\pi_1 + \beta\pi_2 = 0 \implies \alpha\pi_1 = \beta\pi_2$.
...
\begin{frame}[fragile]{Last one did not go well}
    \begin{itemize}
        ...
        \item Interrupted at 291.6007092 because of the test
        \newline (Penultimate slide: sim stopped because the population went extinct, I did not stop it!)
        \item At stop time
        \begin{itemize}
            \item $N =$ \ensuremath{3.7536\times 10^{4}}
            \item $|N| =$ 75035 (and $|t|$ as well, of course!)
...
\end{alltt}
\begin{verbatim}
## [1] 8.998582e-06 8.280973e-04 8.758254e-04 1.810774e-03 1.362021e-03
## [6] 5.322704e-09
\end{verbatim}
...
% gamma = 1/3
% beta = as.numeric(1.5*gamma/(Pop-IC["S"]))
...
% 
% \end{frame}
% %\maxFrameImage{fig_chunk('sim-gillespie2-first', 'pdf')}
% 
% 
% 
...
% 
% 
% %\maxFrameImage{fig_chunk('parallel-CTMC', 'pdf')}
% 
...
% The section page
\newSubSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_hz99k4hz99k4hz99.jpeg}

\begin{frame}{To do}
	\begin{itemize}
		\item Try to work quarantine into the model in a ``non-cohorty'' manner
		\vfill
		\item MBPA to compute probability of an outbreak
		\vfill
		\item Detailed computational analysis of the CTMC
	\end{itemize}
\end{frame}
...
% --- SLIDE 2: What is a Branching Process? ---
\begin{frame}
...
% --- SLIDE 3: The Galton-Watson Process (Formal Definition) ---
\begin{frame}
    \frametitle{The Galton-Watson Process: A Formal View}
